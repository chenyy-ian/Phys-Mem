    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if (current_end > kv_cache["global_end_index"].item()) and (
                ...
                if kv_policy == "hard_evict":
                ...
                elif evict_from_middle == 2:
                    # 3B) 固定丢第 6–8 帧
                    frames_in_cache = max(0, (local_end_old - sink_tokens) // frame_seqlen)
                    evict_frames = min(3, frames_in_cache)

                    if frames_in_cache >= 6:
                        mid_start_frame_idx = 6  # 固定剔除第6–8帧
                    else:
                        # 兜底：不足9帧时，退化为老端驱逐
                        mid_start_frame_idx = 0

                    mid_start = sink_tokens + mid_start_frame_idx * frame_seqlen
                    mid_end   = mid_start + evict_frames * frame_seqlen

                    # 将右侧整体左移覆盖 [mid_start:mid_end)
                    right_len = local_end_old - mid_end
                    if right_len > 0:
                        kv_cache["k"][:, mid_start:mid_start + right_len] = \
                            kv_cache["k"][:, mid_end:mid_end + right_len].clone()
                        kv_cache["v"][:, mid_start:mid_start + right_len] = \
                            kv_cache["v"][:, mid_end:mid_end + right_len].clone()

                    # 压紧后的尾指针
                    local_end_old = local_end_old - evict_frames * frame_seqlen
                elif evict_from_middle == 6:
                    # 3B) 固定丢第 6–8 帧
                    frames_in_cache = max(0, (local_end_old - sink_tokens) // frame_seqlen)
                    evict_frames = min(3, frames_in_cache)

                    if frames_in_cache >= 5:
                        mid_start_frame_idx =1  # 固定剔除第1–3帧
                    else:
                        # 兜底：不足9帧时，退化为老端驱逐
                        mid_start_frame_idx = 0
                    ...
                    # 压紧后的尾指针
                    local_end_old = local_end_old - evict_frames * frame_seqlen
                elif evict_from_middle==1:
                                      
                    frames_in_cache = max(0, (local_end_old - sink_tokens) // frame_seqlen)
                    evict_frames = min(3, frames_in_cache)

                    if frames_in_cache >= 6:
                        mid_start_frame_idx = 3  
                    else:
